...
int process_page_access_fifo(struct PTE page_table[TABLEMAX], int *table_cnt, int page_number, int frame_pool[POOLMAX], int *frame_cnt, int current_timestamp) {
    // Print the status of page table before accessing the page

    
    printf("\nBefore accessing page %d:\n", page_number);
    for (int i = 0; i < TABLEMAX; i++) {
        printf("Page %d: Valid=%d, Frame=%d, Arrival=%d, LastAccess=%d\n",
            i, page_table[i].is_valid, page_table[i].frame_number, page_table[i].arrival_timestamp, page_table[i].last_access_timestamp);
    }
    ...
    if (*frame_cnt > 0) {
        int frame = frame_pool[0];  // Dequeue from front

        // Shift all frames to the left
        ...
        printf("Page fault at page number: %d because the page was not in memory\n", page_number);
    ...
    // Enqueue the freed frame to the end of the pool
    frame_pool[*frame_cnt] = frame;
    (*frame_cnt)++;
...
int count_page_faults_fifo(struct PTE page_table[TABLEMAX], int table_cnt, int reference_string[REFERENCEMAX], int reference_cnt, int frame_pool[POOLMAX], int frame_cnt) {
    ...
    for (int i = 0; i < reference_cnt; i++) {
        ...
        int result = process_page_access_fifo(page_table, &table_cnt, page_number, frame_pool, &frame_cnt, current_timestamp);

        // Increase faults counter if a page fault occurred.
        if (result == -1) {
            faults++;
        }
